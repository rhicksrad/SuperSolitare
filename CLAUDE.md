...
SFX are fully synthesized in `src/ui/audio.ts` (WebAudio; engine/store call `sfx.*`). Music is 25 public-domain MP3s in `public/music/` (see `CREDITS.md` there) looped through WebAudio with a crossfade — one track per round (`a{ante}-{small|big|boss}`, endless cycles antes 6–8) plus a menu/shop lounge. The scene is derived from `screen`+`run.ante`+`run.blindIndex` by a store subscriber at the bottom of `store.ts`; buffers load lazily and the subscriber preloads the current ante's three tracks between rounds. The collection's Sound Test plays any track via the `setJukebox` override (cleared by the subscriber on leaving the collection); tracks unlock by `stats.bestAnte`. An `AnalyserNode` taps the music bus; `MusicGlow.tsx` reads it per frame into `--music-glow`/`--music-bass` CSS vars that light the felt (`#root::after`), disabled by the reduceMotion/music settings. Boss rounds also tint the felt per boss id (`BossWash.tsx`, colors local to that file). End screens chart `run.history` (score vs target per round) via `RunRecap` in `EndScreens.tsx`. Card movement uses a FLIP hook keyed on `data-card-id` in `Board.tsx`; settings (sfx/music/reduceMotion) persist in localStorage separately from saves.
