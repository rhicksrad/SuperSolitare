...
function scoreFoundationPlay(
  ...
  const acc = { chips: base.chips + cardChips(card.rank), mult: base.mult + streakBonus, xmults: [] as number[], money: 0 }
  ...
  if (enh) {
    ...
    for (let t = 0; t < times; t++) {
      ...
      if (enh === 'lucky' && rng.chance(1, 3)) acc.money += 3
    }
  }
...
function scoreEvent(
  ...
  switch (ev.type) {
    ...
    case 'stock_deal': {
      if (boss?.dealPenalty) {
        return { kind: 'penalty', label: `${bossRegistry[round.bossId!].name}`, total: -boss.dealPenalty }
      }
      return null
    }
    case 'board_clear': {
...
function cloneRun(run: RunState): RunState {
  return {
    ...run,
    jokers: run.jokers.map((j) => ({ id: j.id, state: { ...j.state } })),
...
export function performMove(prevRound: RoundState, prevRun: RunState, move: Move): MoveOutcome {
  ...
  // Streak persistence passive (Ouroboros) — undo the engine's reset
  ...
  for (const ev of events) {
    ...
    if (pop.money) run.money += pop.money
